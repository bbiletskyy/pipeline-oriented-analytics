...
class CellToken(Transformer):
    ...
    def __init__(self, level: int, lat_col: str = 'lat', lon_col: str = 'lon', output_col: str = 'cell_id'):
        super(CellToken, self).__init__()
        self._level = level
        self._lat_col = lat_col
        self._lon_col = lon_col
        self._cell_id_col = output_col
        self._cell_id_udf = f.udf(CellToken._cell_token, StringType())
    ...
    @classmethod
    def _cell_token(cls, level: int, lat: int, lon: int) -> str:
